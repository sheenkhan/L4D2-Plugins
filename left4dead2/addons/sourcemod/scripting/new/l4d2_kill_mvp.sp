...
public OnPluginStart()
{
	...
	hCountMvpDelay = CreateConVar("kill_mvp_display_delay", "720", "How long to kill the ranking (seconds).", 0, true, 10.0, true, 9999.0);
...
displaykillinfected()
{
	...
	PrintToChatAll("\x03[MVP]击杀之王:\x04%N \x03击杀数:\x04%i", client, damageffss);
	
	SortCustom1D(players_clients, 24, SortByffDamageDesc);
	damageffss = damageff[players_clients[0]];
	PrintToChatAll("\x03[LVP]黑枪之王:\x04%N \x03黑枪值:\x04%i", players_clients[0], damageffss);
